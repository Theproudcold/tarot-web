    ...
    <div className="w-full max-w-4xl mx-auto mt-8 bg-black/40 backdrop-blur-md rounded-xl border border-tarot-gold/30 p-8 text-left animate-fadeIn">
      ...
      <div className="bg-tarot-gold/10 p-6 rounded-xl border border-tarot-gold/20">
        ...
        <p className="text-gray-200 leading-loose">
          {language === 'zh' ? (
            `你的过去受到了${getLocalized(past.name)}的影响，这意味着${getLocalized(past.meaning_upright)}。
             现在，${getLocalized(present.name)}显现，提示你关注${getLocalized(present.meaning_upright)}。
             展望未来，${getLocalized(future.name)}预示着${getLocalized(future.meaning_upright)}。
             综合来看，这是一个关于转变与成长的旅程，请相信直觉的指引。`
          ) : (
            `Your path began with the energy of ${getLocalized(past.name)}, signifying ${getLocalized(past.meaning_upright)}. 
             Currently, you are embodying ${getLocalized(present.name)}, which brings focus to ${getLocalized(present.meaning_upright)}. 
             Moving forward, ${getLocalized(future.name)} suggests a destiny involving ${getLocalized(future.meaning_upright)}. 
